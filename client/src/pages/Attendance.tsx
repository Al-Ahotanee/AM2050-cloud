...
export default function Attendance({ role }: { role: Role }) {
  ...
  const scan = async (qrToken: string) => {
    setSaving(true);
    const result = await apiClient.request<AttendanceRecord>("/attendance/scan", {
      method: "POST",
      body: {
        qrToken,
        date: form.date,
    ...
    const matched = enrollments.find((item) => item.child_id === result.data.child_id);
    const childName = matched ? `${matched.first_name} ${matched.last_name}` : "Student";
    setFilterDate(form.date);
    await load();
    return {
      success: true,
      childName,
      message: `${childName} marked ${form.attendanceStatus}`,
    };
  };
